...
Full_Tile_Mean = mean(mean(imageData.Cropped_Imaging_Data,1),2);
 ...
 timestamp = block.timestamp; %change this number
  parameters.adjusted_times(:)= block.Sound_Time(:); % this '5' accounts for an apparent 500 ms shift in the data
      figure;   
      hold on;
...
%       data.([mouseID]).(['Tile' BOT_number]).Average_Sound_Response = mean_across_all_trials;
      ...
      estimated_peak = block.setup.framerate*5; %the peak min response should be about 5 sec after stim (1s)
...
baseline=1:(setup.baseline_window*data.setup.FrameRate{1});% TODO: magic number
window=(length(baseline)+1):(data.setup.FrameRate{1}*setup.sound_window_end);
