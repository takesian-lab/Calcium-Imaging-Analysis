...
%% what do the individual traces look like for a tile if the data are zscored here? - this is a work in progress as of July 2021
% zscore the transients as (f - (baseline mean))/std(baseline)
for ll=1:loops
    loop_num=num2str(ll);
    ...
    for f=1:length(parameters.frequencies);
        ...
        for lv=1:length(parameters.levels);
            ...
            if parameters.sort_loco ==0
                idx=parameters.stimIDX{f,lv};
                if lv ==1 & f==1 & ll==1
                    display('..zscore for all trials...')
                end
            else idx = parameters.loco_0.stimIDX{f,lv};
                if lv ==1 & f==1 & ll==1
                    display('...zscore for non-motor trials...')
                end
            end
            
            TF = isempty(idx); %the index of freq x level does not always have a value for every combination
            if TF==0
                StimToZ =  traces.(['Tile' loop_num]){f,lv}(:,:,:,:);;% F for the zscore calculation
                for Zdim4 = 1:size(StimToZ,4); %each trace
                    for Zdim1 = 1:size(StimToZ,1); %all of x
                        for Zdim2 = 1:size(StimToZ,2); %all of y
                            % left off here... need to loop through x and y !
                            basez = squeeze(StimToZ(Zdim1,Zdim2,baseline,Zdim4)); %baseline for zscore calculation
                            meanBaseZ = mean(basez);
                            stdBaseZ = std(basez);
                            Fz = squeeze(StimToZ(Zdim1,Zdim2,:,Zdim4));
                            zscoredtrace = (Fz - meanBaseZ)./stdBaseZ;
                            ZTrace.(['Tile' loop_num]){f,lv}(Zdim1,Zdim2,:,Zdim4) = zscoredtrace(:,:,:);
                        end
                    end
                end
