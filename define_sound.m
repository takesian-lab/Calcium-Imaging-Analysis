function [data]=define_sound(data,setup)
...
for a=1:length(setup.mousename)
    ...
    for i=1:length(Imaging_Block)
        ...
        New_sound_times=data.([mouseID]).(['ImagingBlock' Imaging_Num]).New_sound_times;
        start_time=data.([mouseID]).(['ImagingBlock' Imaging_Num]).start_time;
